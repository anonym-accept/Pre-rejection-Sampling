...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
  unsigned int i;
  uint16_t all_count_abnomal_t0 = 0;
  uint16_t nonce_key = 0;
  ...
  // polyvecl matbuf[K][2];
#endif
  // polyvecl s1,s11, *row = rowbuf;
  polyvecl s1,s11;
  polyvecl mat[K];
  polyveck s2;
  poly t1, t0;
  polyveck t1_store,t0_store;
  ...
  for(i = 0; i < K; i++) {
    ...
    for(unsigned int j = 0; j < L; j++) {
      ...
      poly_uniform_preinit(&(mat[i].vec[j]), &aesctx);
      poly_nttunpack(&(mat[i].vec[j]));
    }
  }
#else
    // polyvec_matrix_expand_row(&row, rowbuf, rho, i);
    // polyvec_matrix_expand_row(&row, matbuf[i], rho, i);
    polyvec_matrix_expand(mat,rho);
#endif


rej_add:
  all_count_abnomal_t0 = 0;
  nonce_key++;
  /* Sample short vectors s1 and s2 */
#ifdef DILITHIUM_USE_AES
  aes256ctr_init(&aesctx, rhoprime, 0+nonce_key);
  for(i = 0; i < L; ++i) {
    nonce = nonce_key + i;
    aesctx.n = _mm_loadl_epi64((__m128i *)&nonce);
...
#elif K == 4 && L == 4
  poly_uniform_eta_4x(&s1.vec[0], &s1.vec[1], &s1.vec[2], &s1.vec[3], rhoprime, 0+nonce_key, 1+nonce_key, 2+nonce_key, 3+nonce_key);
  poly_uniform_eta_4x(&s2.vec[0], &s2.vec[1], &s2.vec[2], &s2.vec[3], rhoprime, 4+nonce_key, 5+nonce_key, 6+nonce_key, 7+nonce_key);
#elif K == 6 && L == 5
  poly_uniform_eta_4x(&s1.vec[0], &s1.vec[1], &s1.vec[2], &s1.vec[3], rhoprime, 0+nonce_key, 1+nonce_key, 2+nonce_key, 3+nonce_key);
  poly_uniform_eta_4x(&s1.vec[4], &s2.vec[0], &s2.vec[1], &s2.vec[2], rhoprime, 4+nonce_key, 5+nonce_key, 6+nonce_key, 7+nonce_key);
  poly_uniform_eta_4x(&s2.vec[3], &s2.vec[4], &s2.vec[5], &t0, rhoprime, 8+nonce_key, 9+nonce_key, 10+nonce_key, 11+nonce_key);
#elif K == 8 && L == 7
  poly_uniform_eta_4x(&s1.vec[0], &s1.vec[1], &s1.vec[2], &s1.vec[3], rhoprime, 0+nonce_key, 1+nonce_key, 2+nonce_key, 3+nonce_key);
  poly_uniform_eta_4x(&s1.vec[4], &s1.vec[5], &s1.vec[6], &s2.vec[0], rhoprime, 4+nonce_key, 5+nonce_key, 6+nonce_key, 7+nonce_key);
  poly_uniform_eta_4x(&s2.vec[1], &s2.vec[2], &s2.vec[3], &s2.vec[4], rhoprime, 8+nonce_key, 9+nonce_key, 10+nonce_key, 11+nonce_key);
  poly_uniform_eta_4x(&s2.vec[5], &s2.vec[6], &s2.vec[7], &t0, rhoprime, 12+nonce_key, 13+nonce_key, 14+nonce_key, 15+nonce_key);
#else
#error
#endif
...
// #endif
    ...
    // printf("all_count_abnomal_t0:%d   ",all_count_abnomal_t0);
    all_count_abnomal_t0 += poly_countAbnomalDataBySigma(&t0);
  ...
  // printf("all_count_abnomal_t0:%d\n",all_count_abnomal_t0);
  if(all_count_abnomal_t0 > N*K*(1-0.95))
    goto rej_add;

      /* Pack secret vectors */
  for(i = 0; i < L; i++)
    polyeta_pack(sk + 3*SEEDBYTES + i*POLYETA_PACKEDBYTES, &s11.vec[i]);

  for(i = 0; i < K; i++)
    polyeta_pack(sk + 3*SEEDBYTES + (L + i)*POLYETA_PACKEDBYTES, &s2.vec[i]);

  for(i = 0; i < K; i++) {
      polyt1_pack(pk + SEEDBYTES + i*POLYT1_PACKEDBYTES, &t1_store.vec[i]);
      polyt0_pack(sk + 3*SEEDBYTES + (L+K)*POLYETA_PACKEDBYTES + i*POLYT0_PACKEDBYTES, &t0_store.vec[i]);
  }
