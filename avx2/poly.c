...
/*************************************************
* Name:        poly_chkAbnomalDataBySigma
*
* Description: Count the coefficients that do NOT locate in range [-2*sigma,2*sigma].
               Input coefficients are in a uniform distribution.
*              Assumes input coefficients were reduced by reduce32().
*
* Arguments:   - const poly *a: pointer to polynomial
*
* Returns count, the number of large ( > 2*sigma or < -2*sigma) coefficients.
**************************************************/
int poly_countAbnomalDataBySigma(const poly *a) {
  unsigned int i;
  int32_t t;
  uint16_t count = 0;
  DBENCH_START();
